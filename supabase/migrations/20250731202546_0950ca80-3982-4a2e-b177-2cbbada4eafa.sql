...
USING (
  EXISTS (
    ...
    WHERE p.id = proof_id AND s.client_id IN (
      SELECT client_id FROM public.submissions WHERE created_by = auth.uid()
    )
  )
);
...
BEGIN
    ...
    IF NEW.status = 'Envoyée au client' AND (OLD.status IS NULL OR OLD.status != 'Envoyée au client') THEN
        ...
        SELECT p.*, o.order_number, s.submission_number, s.client_id, c.business_name, c.contact_email, c.contact_name
        INTO proof_record
