...
-- Idempotent, sans double-insert
-- ======================================================

-- Extensions utiles (déjà présentes, sans danger si rejouées)
CREATE EXTENSION IF NOT EXISTS pgcrypto WITH SCHEMA extensions;

-- Nettoyage d'anciens triggers/funcs si présents
DROP TRIGGER IF EXISTS create_order_on_approval ON public.submissions;
DROP FUNCTION IF EXISTS public.create_order_from_submission();

-- Optionnel: si tu avais un ancien trigger auto-proof au niveau orders
DROP TRIGGER IF EXISTS create_initial_proof_after_order ON public.orders;
DROP FUNCTION IF EXISTS public.create_initial_proof_after_order();
