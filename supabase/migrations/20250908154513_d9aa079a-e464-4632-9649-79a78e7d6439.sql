...
-- Accorder uniquement les permissions nécessaires aux utilisateurs authentifiés
-- Les RLS policies s'occuperont du contrôle fin

-- Logs et monitoring restent accessibles selon leurs propres RLS
-- Les vues publiques peuvent être accordées si nécessaire
...
-- Produits et variants: lecture pour tous, écriture admin
...
-- Commentaires de fin
COMMENT ON SCHEMA public IS 'PromoFlow - Sécurisé selon plan Lovable/Supabase - ' || NOW();