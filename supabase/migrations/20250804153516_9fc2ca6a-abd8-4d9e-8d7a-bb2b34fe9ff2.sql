...
-- 1. Améliorer la table ordre_historique avec des contraintes et colonnes manquantes
ALTER TABLE public.ordre_historique 
ADD CONSTRAINT IF NOT EXISTS check_action_type CHECK (action_type IN (
    'upload_epreuve',
    'send_epreuve', 
    'view_epreuve',
    'approve_epreuve',
...
COMMENT ON VIEW public.v_ordre_historique IS 'Vue enrichie de l''historique des commandes avec toutes les informations jointes';
COMMENT ON FUNCTION public.get_next_proof_version IS 'Fonction utilitaire pour obtenir le prochain numéro de version d''épreuve';

-- 9. Politique RLS pour la vue (les vues héritent des politiques des tables sous-jacentes)
-- Mais on peut ajouter une politique spécifique si nécessaire

-- 10. Créer une fonction pour récupérer l'historique d'une commande spécifique
