import { Users, FileText, Package, Tag, Plus } from "lucide-react"
...
export function BottomNavigation() {
  ...
  const handleFabClick = () => {
    // Navigate to create submission based on current context
    if (currentPath.startsWith("/dashboard/submissions")) {
      navigate("/dashboard/submissions/create")
    } else if (currentPath.startsWith("/dashboard") || currentPath.startsWith("/dashboard/clients")) {
      // Default to creating a submission from clients page
      navigate("/dashboard/submissions/create")
    } else {
      navigate("/dashboard/submissions/create")
    }
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-20 right-6 z-50 w-14 h-14 bg-primary hover:bg-primary/90 text-primary-foreground rounded-full shadow-lg hover:shadow-xl transition-all duration-300 active:scale-95 flex items-center justify-center"
